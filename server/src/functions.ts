...
export const languageFunctions: Record<Language, LanguageFunction> = {
  [Language.ENGLISH]: () => `Respond in English with a mystical yet clear tone, using appropriate fortune telling terminology while keeping the message accessible.`,
  
  [Language.TURKISH]: () => `Türkçe olarak, mistik ama anlaşılır bir tonda yanıt ver. Fal terminolojisini uygun şekilde kullanırken mesajın anlaşılır olmasını sağla.`
};

// Tarot spread-specific functions
export const tarotSpreadFunctions: Record<TarotSpreadType, (language: Language) => string> = {
  [TarotSpreadType.SINGLE_CARD]: (language: Language) => {
    if (language === Language.TURKISH) {
      return `*Tek kart seçiyorum ve önüme yerleştiriyorum* Bu kart sizin için özel bir mesaj taşıyor.`;
    }
    return `*I select a single card and place it before me* This card carries a special message for you.`;
  },
  [TarotSpreadType.THREE_CARD]: (language: Language) => {
    if (language === Language.TURKISH) {
      return `*Üç kart seçiyorum ve sırasıyla geçmiş, şimdi ve geleceği temsil edecek şekilde yerleştiriyorum* Bu kartlar zamanın akışı içinde size rehberlik edecek.`;
    }
    return `*I select three cards and lay them out to represent the past, present, and future* These cards will guide you through the flow of time.`;
  },
  [TarotSpreadType.CELTIC_CROSS]: (language: Language) => {
    if (language === Language.TURKISH) {
      return `*On kartı Kelt Haçı şeklinde diziyorum* Bu güçlü ve kapsamlı dizilim durumunuzun derinliklerine ışık tutacak.`;
    }
    return `*I arrange ten cards in the Celtic Cross formation* This powerful and comprehensive spread will illuminate the depths of your situation.`;
  },
  [TarotSpreadType.HORSESHOE]: (language: Language) => {
    if (language === Language.TURKISH) {
      return `*Yedi kartı at nalı şeklinde diziyorum* Bu dizilim karşılaştığınız zorlukları aşmanız için size yol gösterecek.`;
    }
    return `*I lay out seven cards in a horseshoe pattern* This spread will guide you through overcoming your challenges.`;
  },
  [TarotSpreadType.PENTAGRAM]: (language: Language) => {
    if (language === Language.TURKISH) {
      return `*Beş kartı pentagram şeklinde diziyorum* Bu mistik dizilim elementlerin ve ruhun bilgeliğini size aktaracak.`;
    }
    return `*I arrange five cards in a pentagram formation* This mystical spread will channel the wisdom of the elements and spirit.`;
  },
  [TarotSpreadType.TREE_OF_LIFE]: (language: Language) => {
    if (language === Language.TURKISH) {
      return `*On kartı Hayat Ağacı şeklinde diziyorum* Bu kutsal dizilim ruhsal yolculuğunuza ışık tutacak.`;
    }
    return `*I place ten cards in the Tree of Life formation* This sacred spread will illuminate your spiritual journey.`;
  }
};
